...
class IssueBookView(APIView):
    ...
    def post(self, request):
        ...
        if book.available_count > 0:
            # Decrement the available count and save the book
            ...
            return Response({'message': 'The requested book is not available.'}, status=400)

    # Details of particular issued book based on isbn
    def get(self, request, isbn):
        issued_books = IssuedBook.objects.filter(book__isbn=isbn)
        if not issued_books:
            return Response({'message': 'No books have been issued for this book.'}, status=status.HTTP_204_NO_CONTENT)
...
class ReturnBookView(APIView):
    ...
    def post(self, request, isbn,student_id):
        issued_book = get_object_or_404(IssuedBook, book__isbn=isbn, student=student_id)
